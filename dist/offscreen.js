...
chrome.runtime.onConnect.addListener((port) => {
  ...
  port.onMessage.addListener(async (msg) => {
    ...
    try {
      ...
      const canvas = typeof OffscreenCanvas !== "undefined" ? new OffscreenCanvas(tileW, totalH) : Object.assign(document.createElement("canvas"), { width: tileW, height: totalH });
      ...
      for (let i = 0; i < tiles.length; i++) {
        ...
        if (i === 0) {
          ...
          ctx.drawImage(img, 0, yDraw - overlapPx);
          yDraw += tileH - overlapPx;
        }
      }
      async function toDataURLFromCanvas(cnv, type, q) {
